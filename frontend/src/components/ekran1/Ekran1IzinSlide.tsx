...
import { formatLeaveDateRange, normalizeLeaveReason, sortTvLeaves, TV_MAX_LEAVE_ROWS } from "@/lib/izinTvBoard";
...
export function Ekran1IzinSlide({ leaves, loading, error, lastUpdated }: Props) {
  ...
  const stats = useMemo(() => {
    const pending = leaves.filter((l) => l.status === "beklemede").length;
    const approved = leaves.filter((l) => l.status === "onaylandi").length;
