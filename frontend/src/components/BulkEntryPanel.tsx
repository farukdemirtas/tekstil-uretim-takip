...
export default function BulkEntryPanel({ rows, onApply }: BulkEntryPanelProps) {
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-4">
      <h2 className="text-base font-semibold">Toplu Veri Girişi</h2>
      <p className="mt-1 text-sm text-slate-600">
        Excel&apos;den satırları seçip yapıştırın. Format: <code>Ad Soyad[TAB]10:00[TAB]13:00[TAB]16:00[TAB]18:30</code>
      </p>
      <textarea
        value={text}
      ...
      <div className="mt-3 flex items-center gap-3">
        <button onClick={() => void handleApply()} className="rounded-md bg-indigo-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700">
          Toplu Kaydet
        </button>
