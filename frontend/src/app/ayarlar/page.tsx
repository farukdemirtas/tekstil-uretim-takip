...
export default function AyarlarPage() {
  ...
  async function load() {
    setLoading(true);
    try { setList(await getWorkerNames()); }
  ...
  async function handleAdd(e: FormEvent) {
    ...
    if (!trimmed) { setAddError("İsim boş olamaz."); return; }
    setAddError("");
    setAddBusy(true);
    try {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-3xl flex-col gap-5 p-4 md:p-8">

      {/* Başlık */}
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold">Ayarlar — İşçi İsim Listesi</h1>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              İsim ekleyebilir, düzenleyebilir ve silebilirsiniz.
              Ana ekrandaki "Çalışan Ekle" formu bu listeden beslenir.
            </p>
          </div>
          <Link
            href="/"
      ...
      {/* Yeni İsim Ekle */}
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <h2 className="mb-3 text-sm font-semibold text-slate-700 dark:text-slate-200">Yeni İsim Ekle</h2>
      ...
      <section className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
        ...
        {loading ? (
          ...
          <ul className="divide-y divide-slate-100 dark:divide-slate-700">
            {filtered.map((w, idx) => (
              <li key={w.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-700/40">
                ...
                {editId === w.id ? (
                  <input
                    ...
                    className="flex-1 rounded border border-blue-400 px-2 py-1 text-sm uppercase outline-none dark:border-blue-500 dark:bg-slate-700 dark:text-slate-100"
                  />
                ) : (
